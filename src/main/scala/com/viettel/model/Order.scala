 ...
 */
class Order private (var orderId: Long, var userId: Long, var total: Double, var orderItems: ArrayBuffer[OrderItem], var created_time: Long) {
  def this(orderId: Long, total: Double, created_time: Long) {
...
object Order extends HBaseCommonUtils[Order] {
  ...
  def of(rs: Result): Order = {
    ...
    new Order(Bytes.toLong(orderId), Bytes.toDouble(total), Bytes.toLong(createdTime))
  }

  override def hbaseRowKey(order: Order): Array[Byte] = {
    Bytes.toBytes(s"+${order.created_time}+${order.orderId}")
  }
}